...
/**
 * Sample an arc that opens upward — a low-frequency cosine envelope shaped
 * by a wider Gaussian-like falloff at the edges. The bell peak now hangs at
 * the bottom of the field so strand bodies rain down from the top into the
 * row of control towers anchored at the bottom of the canvas.
 */
export const sampleArc = (p: ArcParams): ArcSample => {
  ...
  const y =
    (1 - p.centerY) * p.height +
    env * p.amplitude * p.height -
    wobble * p.amplitude * p.height * 0.18;
  return { x, y };
};
